...
export default function AdditionalPage() {
  ...
  const handleDownload = async () => {
    ...
    const fetchPromises = files.map(({ node }, index) => {
      ...
      if (node.image?.url) {
        ...
        fileName = extractFileName(fileUrl).replace(/\.[^/.]+$/, "") + ".webp"; // Change extension to WebP
            ...
            return new Promise((resolve) => {
              img.onload = () => {
                ...
                canvas.toBlob((webpBlob) => {
                  imageFolder.file(fileName, webpBlob);
                  completedFetches++;
  ...
  return (
    <Page title="FileMaster - WebP Converter">
      <ui-title-bar title="FileMaster - WebP Converter"></ui-title-bar>
      <Frame>
        <Layout>
          <Layout.Section>
                    ...
                    <Box paddingBlockStart="100">
                      <InlineStack align="center">
                        <Button
                          ...
                          icon={ImportIcon}
                        >
                          {isDownloading ? "Downloading..." : "WebP Converter"}
                        </Button>
                      </InlineStack>
                    </Box>
                  </BlockStack>
